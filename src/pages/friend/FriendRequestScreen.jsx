...
export default function FriendRequestScreen() {
  ...
  console.log("[FriendRequestScreen] location.state:", state);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      await sendFriendRequest(targetId, message.trim());
      ...
      alert(`친구 신청 보내기에 실패했어요.\n\n서버 응답: ${backendMessage}`);
    } finally {
      setSending(false);
    }
  };
  ...
  return (
    <div className="flex flex-col h-screen bg-white font-[Pretendard]">
      ...
      <div className="flex-1 overflow-y-auto w-full max-w-[500px] mx-auto pb-[6rem]">
        {/* 프로필 */}
        <div className="mt-[3.75rem] flex flex-col items-center">
          <div className="w-[4.75rem] h-[4.75rem] rounded-full bg-[#E5E7EB] overflow-hidden">
            <img
              src={profileImage}
              alt="프로필"
          ...
          <p className="mt-[1rem] text-[1rem] font-semibold text-[#191D1F]">
            {nickname}
          </p>
        </div>
        ...
        <div className="mt-[2.5rem] px-[1.5rem]">
          <div className="border border-[#D1D5DB] rounded-[1rem] px-[1rem] py-[0.75rem] min-h-[7rem]">
            <textarea
              className="w-full h-full resize-none outline-none border-none text-[1rem] text-[#111827] placeholder:text-[#D1D5DB]"
              value={message}
          ...
          <p className="mt-[0.625rem] text-[0.75rem] text-[#6B7280] leading-[1.4rem]">
      ...
      <div className="px-[1.5rem] pb-[2.25rem]">
        <button
          disabled={disabled}
          onClick={handleSend}
          className={`w-full h-[3rem] rounded-[0.5rem] text-[1rem] font-bold ${
